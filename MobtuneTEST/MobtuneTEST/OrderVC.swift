...
class OrderViewController: UIViewController {
    ...
    @objc private func confirmAction() {
        let orderData = ProductInfo()
        
        orderData.orderId = idInputfield.text!
        orderData.totalPrice = priceInputField.text!
        
        if methodBtn.tag == 0 {
            orderData.paymentMethod = .CREDICTCARD
        }else if methodBtn.tag == 1 {
            orderData.paymentMethod = .BANKTRANSFER
        }else if methodBtn.tag == 2 {
            orderData.paymentMethod = .MOBILEPAYMENT
        }else {
            orderData.paymentMethod = .ETC
        }
        ...
        orderData.productItems = productList
        Mobtune.addEvent(eventType: .ORDER, param: orderData)
